...
def eeg(signal=None, sampling_rate=1000., labels=None, show=True):
    ...
    if labels is None:
        ...
        if len(labels) != nch:
            raise ValueError("Number of channels mismatch between signal matrix and labels.")
    
    # high pass filter
    b, a = st.get_filter(ftype='butter', band='highpass', order=8, frequency=4,
                         sampling_rate=sampling_rate)
    
    aux, _ = st._filter_signal(b, a, signal=signal, check_phase=True, axis=0)
    
    # low pass filter
    b, a = st.get_filter(ftype='butter', band='lowpass', order=16, frequency=40,
                         sampling_rate=sampling_rate)
    
    filtered, _ = st._filter_signal(b, a, signal=aux, check_phase=True, axis=0)
    
    # band power features
    out = get_power_features(signal=filtered, sampling_rate=sampling_rate, size=0.25, overlap=0.5)
    ts_feat = out['ts']
    theta = out['theta']
    ...
    # PLF features
    _, plf_pairs, plf = get_plf_features(signal=filtered, sampling_rate=sampling_rate, size=0.25, overlap=0.5)
    ...
    # plot
    if show:
        plotting.plot_eeg(ts=ts, raw=signal, filtered=filtered, labels=labels,
                          features_ts=ts_feat, theta=theta, alpha_low=alpha_low,
                          alpha_high=alpha_high, beta=beta, gamma=gamma,
                          plf_pairs=plf_pairs, plf=plf, path=None, show=True)
    
    # output
    args = (ts, filtered, ts_feat, theta, alpha_low, alpha_high, beta, gamma, plf_pairs, plf)
    names = ('ts', 'filtered', 'features_ts', 'theta', 'alpha_low', 'alpha_high', 'beta', 'gamma', 'plf_pairs', 'plf')
...
def get_power_features(signal=None, sampling_rate=1000., size=0.25, overlap=0.5):
    ...
    Args:
        ...
        size (float): Window size (seconds) (optional).
    ...
    # windower
    fcn_kwargs = {'sampling_rate': sampling_rate,
                  'bands': bands,
                  'pad': pad,
                  }
    index, values = st.windower(signal=signal, size=size, step=step, kernel='hann',
                                fcn=_power_features, fcn_kwargs=fcn_kwargs)
    ...
    for i in xrange(nb):
        for j in xrange(nch):
            values[:, i, j], _ = st.smoother(signal=values[:, i, j], kernel='median', size=md_size)
...
def get_plf_features(signal=None, sampling_rate=1000., size=0.25, overlap=0.5):
    ...
    # windower
    fcn_kwargs = {'pairs': pairs,
                  'N': N,
                  }
    index, values = st.windower(signal=signal, size=size, step=step, kernel='hann',
                                fcn=_plf_features, fcn_kwargs=fcn_kwargs)
    ...
    for i in xrange(nb):
        values[:, i], _ = st.smoother(signal=values[:, i], kernel='median', size=md_size)
...
def _power_features(signal=None, sampling_rate=1000., bands=None, pad=0):
    ...
    Returns:
        out (array): Average power for each band and EEG channel; shape is (bands, channels).
    ...
    for i in xrange(nch):
        ...
        freqs, power = st.power_spectrum(signal=signal[:, i], sampling_rate=sampling_rate,
                                         pad=pad, pow2=False, decibel=False)
