...
def resp(signal=None, sampling_rate=1000., show=True):
    ...
    # filter signal
    filtered, _, _ = st.filter_signal(signal=signal, ftype='butter', band='bandpass',
                                      order=2, frequency=[0.1, 0.35], sampling_rate=sampling_rate)
    ...
    if len(beats) < 2:
        ...
        size = 3
        rate, _ = st.smoother(signal=rate, kernel='boxcar', size=size, mirror=True)
    ...
    # plot
    if show:
        plotting.plot_resp(ts=ts, raw=signal, filtered=filtered, zeros=zeros,
                           resp_rate_ts=ts_rate, resp_rate=rate,
                           path=None, show=True)
