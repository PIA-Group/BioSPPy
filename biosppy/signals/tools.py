...
def _filter_signal(b, a, signal, zi=None, check_phase=True, **kwargs):
    ...
    if check_phase and zi is not None:
        raise ValueError("Incompatible arguments: initial filter state cannot be set when check_phase is True.")
...
def _get_window(kernel, size, **kwargs):
    ...
    if kernel in ['blackman', 'black', 'blk']:
        ...
        winfunc = ss.gaussian
    elif kernel in ['general gaussian', 'general_gaussian',
...
def get_filter(ftype='FIR', band='lowpass', order=None, frequency=None, sampling_rate=1000., **kwargs):
    ...
    if band not in ['lowpass', 'highpass', 'bandpass', 'bandstop']:
        raise ValueError("Unknown filter type '%r'; choose 'lowpass', 'highpass', 'bandpass', or 'bandstop'." % band)
    ...
    if ftype == 'FIR':
        ...
        # Butterworth filter
        b, a = ss.butter(N=order, Wn=frequency, btype=band, analog=False, output='ba', **kwargs)
    elif ftype == 'cheby1':
        # Chebyshev type I filter
        b, a = ss.cheby1(N=order, Wn=frequency, btype=band, analog=False, output='ba', **kwargs)
    elif ftype == 'cheby2':
        # chevyshev type II filter
        b, a = ss.cheby2(N=order, Wn=frequency, btype=band, analog=False, output='ba', **kwargs)
    elif ftype == 'ellip':
        # Elliptic filter
        b, a = ss.ellip(N=order, Wn=frequency, btype=band, analog=False, output='ba', **kwargs)
    elif ftype == 'bessel':
        # Bessel filter
        b, a = ss.bessel(N=order, Wn=frequency, btype=band, analog=False, output='ba', **kwargs)
    
    return utils.ReturnTuple((b, a), ('b', 'a'))


def filter_signal(signal=None, ftype='FIR', band='lowpass', order=None,
                  frequency=None, sampling_rate=1000., **kwargs):
    ...
    # get filter
    b, a = get_filter(ftype=ftype, order=order, frequency=frequency,
                      sampling_rate=sampling_rate, band=band, **kwargs)
...
def smoother(signal=None, kernel='boxzen', size=10, mirror=True, **kwargs):
    ...
    Args:
        ...
        kernel (str, array): Type of kernel to use; if array, use directly as the kernel.
        
        size (int): Size of the kernel; ignored if kernel is an array.
        
        mirror (bool): If True, signal edges are extended to avoid boundary effects (optional).
    ...
    w = win / win.sum()
    if mirror:
        aux = np.concatenate((signal[0] * np.ones(size), signal, signal[-1] * np.ones(size)))
...
def power_spectrum(signal=None, sampling_rate=1000., pad=None, pow2=False, decibel=True):
    ...
    Args:
        ...
        pow2 (bool): If True, rounds the number of points N = len(signal) + pad to the nearest power of 2 greater than N (optional).
...
def band_power(freqs=None, power=None, frequency=None, decibel=True):
    ...
    if len(freqs) != len(power):
        raise ValueError("The input 'freqs' and 'power' arrays must have the same length.")
...
def signal_stats(signal=None):
    ...
    names = ('mean', 'median', 'max', 'var', 'std_dev', 'abs_dev', 'kurtosis', 'skewness')
...
def windower(signal=None, size=None, step=None, fcn=None, fcn_kwargs=None,
             kernel='boxcar', kernel_kwargs=None):
    ...
    Args:
        ...
        kernel_kwargs (dict): Additional keyword arguments to pass on window creation;
...
def _pdiff(x, p1, p2):
    """Compute the squared difference between two interpolators, given the x-coordinates.
...
def find_intersection(x1=None, y1=None, x2=None, y2=None, alpha=1.5, xtol=1e-6, ytol=1e-6):
    ...
    Args:
        ...
        alpha (float): Resolution factor for the x-axis; fraction of total number of x-coordinates (optional).
    ...
    if x1.shape != y1.shape:
        raise ValueError("Input coordinates for the first line must have the same shape.")
    if x2.shape != y2.shape:
        raise ValueError("Input coordinates for the second line must have the same shape.")
    ...
    roots = set()
    for v in xi:
        root, _, ier, _ = optimize.fsolve(_pdiff, v, (p1, p2), full_output=True, xtol=xtol)
